...
ask_shift db "Enter shift value: ",0
len_ask_shift equ $-ask_shift
...
method:
	push ebp
	mov ebp, esp
	mov eax, [esp+0x8]	;массив выбора
	mov ecx, [esp+0xc]	;его длина
	xor esi, esi
	mov bl, [eax]
...
.check_cipher:
	mov ebx, [esp+0x18]	;массив шаблона
	mov edx, [esp+0x1c]	;его длина
	jmp .check_length
	
	
.check_decipher:
	mov ebx, [esp+0x10]	;массив шаблона
	mov edx, [esp+0x14]	;его длина
...
check_passw:
	push ebp
	mov ebp, esp
	mov eax, [esp+0x8]	;correct_passw
	mov ecx, [esp+0xc]	;cor_pass_len
	mov ebx, [esp+0x10]	;entered_password
	mov edx, [esp+0x14]	;entered_passw_len
	dec edx
	xor edi, edi
	cmp ecx, edx
	je .m1
